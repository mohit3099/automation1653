...
resource "azurerm_linux_virtual_machine" "n01581653-vmlinux" {
  ...
  availability_set_id = azurerm_availability_set.n01581653-vmlinux-avs.id

  network_interface_ids = [azurerm_network_interface.n01581653-vmlinux-nic[each.key].id]
  ...
  depends_on = [azurerm_availability_set.n01581653-vmlinux-avs]

  tags = var.common_tags
}

resource "azurerm_availability_set" "n01581653-vmlinux-avs" {
  resource_group_name = var.resource_group_name
  location            = var.location
...
resource "azurerm_public_ip" "n01581653-vmlinux-pip" {
  resource_group_name = var.resource_group_name
  location            = var.location
...
resource "azurerm_network_interface" "n01581653-vmlinux-nic" {
  resource_group_name = var.resource_group_name
  location            = var.location
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.n01581653-vmlinux-pip[each.key].id
  }
...
resource "azurerm_virtual_machine_extension" "n01581653-vmlinux-network-watcher" {
  for_each = local.instances

  name               = "${var.vmlinux-info.name}-network-watcher-${each.key}"
  virtual_machine_id = azurerm_linux_virtual_machine.n01581653-vmlinux[each.key].id
...
resource "azurerm_virtual_machine_extension" "n01581653-vmlinux-network-monitor" {
  for_each = local.instances
